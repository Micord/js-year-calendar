...
export default class Calendar<T extends CalendarDataSourceElement> {
   ...
   *
   * Don't forget to enable the `enableRangeSelection` option to be able to use the range selection
   * functionality.
   * @event
   * @example
   * ```
   ...
   * @param element The element (or the selector to an element) in which the calendar should be
   *     created.
  ...
  protected _initializeOptions(opt: any): void {
    ...
    this.options = {
      ...
      language: (opt.language != null && Calendar.locales[opt.language] != null)
                ? opt.language
                : 'en',
      allowOverlap: opt.allowOverlap != null ? opt.allowOverlap : true,
      displayWeekNumber: opt.displayWeekNumber != null ? opt.displayWeekNumber : false,
      displayDisabledDataSource: opt.displayDisabledDataSource != null
      ...
      dataSource: opt.dataSource instanceof Array || typeof opt.dataSource === "function"
                  ? opt.dataSource
                  : [],
      ...
      customDataSourceRenderer: typeof opt.customDataSourceRenderer === "function"
                                ? opt.customDataSourceRenderer
                                : null,
      weekStart: !isNaN(parseInt(opt.weekStart)) ? parseInt(opt.weekStart) : null,
      loadingTemplate: typeof opt.loadingTemplate === "string" || opt.loadingTemplate instanceof
                       HTMLElement ? opt.loadingTemplate : null
    };
  ...
  protected _initializeEvents(opt): void {
    ...
    if (opt.dblclickDay) {
      this.element.addEventListener('dblclickDay', opt.dblclickDay);
    }
  ...
  protected _renderHeader(): void {
    ...
    if (this.options.minDate != null
        && this.options.minDate > this.getDateUTC(this.options.startYear - 1, 11, 31)) {
    ...
    if (this.options.minDate != null
        && this.options.minDate > this.getDateUTC(this.options.startYear - 2, 11, 31)) {
    ...
    if (this.options.maxDate != null
        && this.options.maxDate < this.getDateUTC(this.options.startYear + 1, 0, 1)) {
    ...
    if (this.options.maxDate != null
        && this.options.maxDate < this.getDateUTC(this.options.startYear + 2, 0, 1)) {
  ...
  protected _renderBody(): void {
    ...
    for (var m = 0; m < 12; m++) {
      ...
      var firstDate = this.getDateUTC(this.options.startYear, m, 1);
      ...
      var lastDate = this.getDateUTC(this.options.startYear, m + 1, 0);
      ...
      while (currentDate <= lastDate) {
        ...
        if (this.options.displayWeekNumber) {
          ...
          var currentThursday = new Date(currentDate.getTime()); // Week number is computed based
                                                                 // on the thursday
  ...
  protected _renderDataSource(): void {
    if (this._dataSource != null && this._dataSource.length > 0) {
      this.element.querySelectorAll('.month-container').forEach((month: HTMLElement) => {
        ...
        var firstDate = this.getDateUTC(this.options.startYear, monthId, 1);
        var lastDate = this.getDateUTC(this.options.startYear, monthId + 1, 1);

        if ((this.options.minDate == null || lastDate > this.options.minDate) &&
            (this.options.maxDate == null || firstDate <= this.options.maxDate)) {
          var monthData = [];

          for (var i = 0; i < this._dataSource.length; i++) {
            if (!(this._dataSource[i].startDate >= lastDate) ||
                (this._dataSource[i].endDate < firstDate)) {
          ...
          if (monthData.length > 0) {
            month.querySelectorAll('.day-content').forEach((day: HTMLElement) => {
              var currentDate = this.getDateUTC(this.options.startYear, monthId,
                                                parseInt(day.textContent));
              var nextDate = this.getDateUTC(this.options.startYear, monthId,
                                             currentDate.getDate() + 1);

              var dayData = [];

              if ((this.options.minDate == null || currentDate >= this.options.minDate) &&
                  (this.options.maxDate == null || currentDate <= this.options.maxDate)) {
                ...
                if (dayData.length > 0 &&
                    (this.options.displayDisabledDataSource || !this._isDisabled(currentDate))) {
        ...
        if (events[events.length - 1].startDate.getTime() == currentTime) {
          ...
          if (events[events.length - 1].startHalfDay || this.options.alwaysHalfDay) {
            ...
            for (var i = events.length - 2; i >= 0; i--) {
              if (events[i].startDate.getTime() != currentTime ||
            ...
            parent.style.background = `linear-gradient(-45deg, ${events[events.length -
                                                                        1].color}, ${events[events.length -
                                                                                            1].color} 49%, ${otherColor} 51%, ${otherColor})`;
          }
          ...
          if (events[events.length - 1].endHalfDay || this.options.alwaysHalfDay) {
            ...
            var otherColor = 'transparent';
            for (var i = events.length - 2; i >= 0; i--) {
              if (events[i].endDate.getTime() != currentTime ||
            ...
            parent.style.background = `linear-gradient(135deg, ${events[events.length -
                                                                        1].color}, ${events[events.length -
                                                                                            1].color} 49%, ${otherColor} 51%, ${otherColor})`;
          }
          else if (this.options.roundRangeLimits) {
          ...
          if (this._mouseDown) {
            ...
            if (!this.options.allowOverlap) {
              ...
              if (newDate < currentDate) {
                var nextDate = this.getDateUTC(newDate.getFullYear(), newDate.getMonth(),
                                               newDate.getDate() + 1);
      ...
      window.addEventListener('mouseup', e => {
        if (this._mouseDown) {
          ...
          this._triggerEvent('selectRange', {
            startDate: minDate,
            endDate: maxDate,
            events: this.getEventsOnRange(
                minDate,
                this.getDateUTC(maxDate.getFullYear(), maxDate.getMonth(), maxDate.getDate() + 1)
            )
          });
        }
      });
  ...
  protected _refreshRange(): void {
    ...
    this.element.querySelectorAll('td.day.range-start')
        .forEach(day => day.classList.remove('range-start'));
    this.element.querySelectorAll('td.day.range-end')
        .forEach(day => day.classList.remove('range-end'));
  ...
  protected _renderContextMenuItems(parent: HTMLElement, items: CalendarContextMenuItem<T>[],
                                    evt: T): void {
    ...
    for (var i = 0; i < items.length; i++) {
      if (items[i].visible === false ||
          (typeof items[i].visible === "function" && !(items[i] as any).visible(evt))) {
        continue;
      }
      ...
      if (items[i].click) {
        (function (index) {
          menuItemContent.addEventListener('click', () => {
            (document.querySelector('.calendar-context-menu') as HTMLElement).style.display =
                'none';
  ...
  protected _getDate(elt): Date {
    ...
    return this.getDateUTC(year, month, day);
  }
  ...
  protected _isDisabled(date: Date): boolean {
    if ((this.options.minDate != null && date < this.options.minDate) ||
  ...
  public getWeekNumber(date: Date): number {
    ...
    var week1 = this.getDateUTC(workingDate.getFullYear(), 0, 4);
  ...
  public getEvents(date: Date): T[] {
    return this.getEventsOnRange(
        date,
        this.getDateUTC(date.getFullYear(), date.getMonth(), date.getDate() + 1)
    );
  }
   ...
   *
   * @param date The specified day.
   * @param after Whether to check for a free slot on the first part (if `false`) or the last
   *     part (if `true`) of the day. If `null`, this will check on the whole day.
   *
   * Usefull only if using the `alwaysHalfDay` option of the calendar, or the `startHalfDay` or
   *     `endHalfDay` option of the datasource.
   */
  public isThereFreeSlot(date: Date, after: Boolean = null): Boolean {
    ...
    if (after === true) {
      return !events.some(
          evt => (!this.options.alwaysHalfDay && !evt.endHalfDay) || evt.endDate > date);
    }
    else if (after === false) {
      return !events.some(
          evt => (!this.options.alwaysHalfDay && !evt.startHalfDay) || evt.startDate < date);
    }
    else {
   ...
   * @param preventRedering Indicates whether the rendering should be prevented after the property
   *     update.
   */
   ...
   * This method causes a refresh of the calendar.
   *
   * @param style The style to use for displaying data source ("background", "border" or
   *     "custom").
   * @param preventRedering Indicates whether the rendering should be prevented after the property
   *     update.
   */
  public setStyle(style: string, preventRendering: boolean = false): void {
    this.options.style =
        style == 'background' || style == 'border' || style == 'custom' ? style : 'border';
  ...
  /**
   * Gets a value indicating whether the user can select a range which overlapping an other
   * element present in the datasource.
   */
  ...
  /**
   * Sets a value indicating whether the user can select a range which overlapping an other
   * element present in the datasource.
   *
   * @param allowOverlap Indicates whether the user can select a range which overlapping an other
   *     element present in the datasource.
   */
   ...
   * This method causes a refresh of the calendar.
   *
   * @param displayDisabledDataSource Indicates whether the data source must be rendered on
   *     disabled days.
   * @param preventRedering Indicates whether the rendering should be prevented after the property
   *     update.
   */
  public setDisplayDisabledDataSource(displayDisabledDataSource: boolean,
                                      preventRendering: boolean = false): void {
  ...
  /**
   * Gets a value indicating whether the beginning and the end of each range should be displayed
   * as half selected day.
   */
  ...
  /**
   * Sets a value indicating whether the beginning and the end of each range should be displayed
   * as half selected day.
   *
   * This method causes a refresh of the calendar.
   *
   * @param alwaysHalfDay Indicates whether the beginning and the end of each range should be
   *     displayed as half selected day.
   * @param preventRedering Indicates whether the rendering should be prevented after the property
   *     update.
   */
   ...
   *     update.
   */
  public setEnableRangeSelection(enableRangeSelection: boolean,
                                 preventRendering: boolean = false): void {
   ...
   * This method causes a refresh of the calendar.
   *
   * @param roundRangeLimits Indicates whether the beginning and the end of each range should be
   *     displayed as rounded cells.
   * @param preventRedering Indicates whether the rendering should be prevented after the property
   *     update.
   */
  ...
  /**
   * Gets a value indicating whether the default context menu must be displayed when right
   * clicking on a day.
   */
  ...
  /**
   * Sets a value indicating whether the default context menu must be displayed when right
   * clicking on a day.
   *
   * This method causes a refresh of the calendar.
   *
   * @param enableContextMenu Indicates whether the default context menu must be displayed when
   ...
   *     update.
   */
  public setContextMenuItems(contextMenuItems: CalendarContextMenuItem<T>[],
                             preventRendering: boolean = false): void {
   ...
   * This method causes a refresh of the calendar.
   *
   * @param handler The function used to render the days. This function is called during render for
   *     each day.
   * @param preventRedering Indicates whether the rendering should be prevented after the property
   *     update.
   */
  public setCustomDayRenderer(customDayRenderer: (element: HTMLElement, currentDate: Date) => void,
                              preventRendering: boolean = false): void {
    this.options.customDayRenderer =
        typeof customDayRenderer === "function" ? customDayRenderer : null;
   ...
   */
  public getCustomDataSourceRenderer(): (element: HTMLElement, currentDate: Date,
                                         events: T[]) => void {
   ...
   * This method causes a refresh of the calendar.
   *
   * @param handler The function used to render the data source. This function is called during
   *     render for each day containing at least one event.
   * @param preventRedering Indicates whether the rendering should be prevented after the property
   *     update.
   */
  public setCustomDataSourceRenderer(customDataSourceRenderer: (element: HTMLElement,
                                                                currentDate: Date,
                                                                events: T[]) => void,
                                     preventRendering: boolean = false): void {
    this.options.customDataSourceRenderer =
        typeof customDataSourceRenderer === "function" ? customDataSourceRenderer : null;
   ...
   */
  public getDataSource(): T[] | ((currentYear: number) => T[] | Promise<T[]>) | ((currentYear: number,
   ...
   *     update.
   */
  public setDataSource(dataSource: T[] | ((currentYear: number) => T[] | Promise<T[]>) | ((currentYear: number,
                                                                                           done: (result: T[]) => void) => void),
                       preventRendering: boolean = false): void {
    this.options.dataSource =
        dataSource instanceof Array || typeof dataSource === "function" ? dataSource : [];
   ...
   * This method causes a refresh of the calendar.
   *
   * @param weekStart The starting day of the week. This option overrides the parameter define in
   *     the language file.
   * @param preventRedering Indicates whether the rendering should be prevented after the
   *     property update.
   */
  public setWeekStart(weekStart: number | string, preventRendering: boolean = false): void {
    this.options.weekStart =
        !isNaN(parseInt(weekStart as string)) ? parseInt(weekStart as string) : null;
  ...
  public setLoadingTemplate(loadingTemplate: string | HTMLElement): void {
    this.options.loadingTemplate =
        typeof loadingTemplate === "string" || loadingTemplate instanceof HTMLElement
        ? loadingTemplate
        : null;
  }
   ...
   * @param preventRendering Indicates whether the calendar shouldn't be refreshed once the event
   *     added.
   */
  ...
  private getDateUTC(year: number, month: number, date: number): Date {
...
if (typeof window === "object") {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll('[data-provide="calendar"]')
        .forEach((element: HTMLElement) => new Calendar(element));
  });
}
